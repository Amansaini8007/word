...
$redirecturl=admin_url('admin.php?page=fb-post-data-settings&auth=1');
$lnredirecturl=admin_url('admin.php?page=fb-post-data-settings&auth=3');
...
// 		}while(array_key_exists("next", $pagearray1->paging));
	...
	else
	{
		...
		if($xyz_fbap_af==1){
			header("Location:".admin_url('admin.php?page=fb-post-data-settings&msg=3'));
			exit();
		}
	}
}
else {
	//header("Location:".admin_url('admin.php?page=fb-post-data-settings&msg=2'));
	//exit();
}
